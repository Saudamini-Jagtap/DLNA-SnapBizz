...
public class ControlActivity extends Activity implements OnClickListener {
	...
	public int previousTimeStamp = 0;
			...
			try {
				...
				if (paramIntent.getAction().equals(Action.PLAY_UPDATE)) {
					if (isUpdatePlaySeek) {
						...
						String counter = localBundle.getString("counter");

						int i = Utils.getRealTime(str1);
						int j = Utils.getRealTime(str2);

						if(counter.contains("Repeat") && (done == false)){
							done = true;
							dmcControl.rePlayControl();
						}
						if(counter.contains("Not")){
							done = false;
						}
	...
	private void initData(Intent localIntent) {
		...
		if(dmcControl != null) {
			if (localIntent.getBooleanExtra("restart", true)) {
				dmcControl.stop(true);
				dmcControl = null;
			}
		}
