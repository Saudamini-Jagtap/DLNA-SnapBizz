...
public class DMCControl {
	...
	public DMCControl(MediaService mediaService, int paramInt,
	...
	private void setPlayErrorMessage() {
		...
		mMediaService.sendBroadcast(localIntent);
	}
	...
	public void getVolume(int paramInt) {
		try {
			...
			if (localService != null) {
				...
				this.upnpService.getControlPoint().execute(
						new GetVolumeCallback(mMediaService, mHandle, paramInt,
...
//			return;
	...
	public void seekBarPosition(String paramString) {
		try {
			...
			if (localService != null) {
				this.upnpService.getControlPoint().execute(
						new SeekCallback(mMediaService, localService, paramString,
								mHandle));
			} else {
				Log.e("null", "null");
			}
